import { IframePreview } from "@/components/iframe-preview";
import { CopyButton } from "./copy-button";
import { ExternalLink } from "lucide-react";

const examples = [
  {
    slug: "chat",
    title: "Chat",
    description: "Full-page chat with sidebar, sessions, and markdown rendering.",
    command: "npx create-polpo-app my-chat -t chat -y",
    source: "https://github.com/lumea-labs/polpo-ui/tree/main/examples/chat",
    codePath: "examples/chat/app/chat/layout.tsx",
    previewUrl: "https://polpo-example-chat.vercel.app",
  },
  {
    slug: "chat-widget",
    title: "Chat Widget",
    description: "Floating/embedded support widget with multiple variants.",
    command: "npx create-polpo-app my-widget -t chat-widget -y",
    source: "https://github.com/lumea-labs/polpo-ui/tree/main/examples/chat-widget",
    codePath: "examples/chat-widget/app/page.tsx",
    previewUrl: "https://polpo-example-chat-widget.vercel.app",
  },
  {
    slug: "multi-agent",
    title: "Multi-Agent",
    description: "Multi-agent workspace with grouped sessions and dark theme.",
    command: "npx create-polpo-app my-workspace -t multi-agent -y",
    source: "https://github.com/lumea-labs/polpo-ui/tree/main/examples/multi-agent",
    codePath: "examples/multi-agent/app/chat/layout.tsx",
    previewUrl: "https://polpo-example-multi-agent.vercel.app",
  },
];

export default function ExamplesPage() {
  return (
    <div className="mx-auto w-full max-w-[1400px] px-6 py-10">
      <p className="font-mono text-[11px] uppercase tracking-[0.25em] text-fd-primary">
        Examples
      </p>
      <h1 className="mt-4 text-3xl font-extrabold tracking-tight md:text-4xl">
        Starter projects
      </h1>
      <p className="mt-3 text-sm text-fd-muted-foreground">
        Each example is a standalone Next.js app. Clone with one command, add your API key, and run.
      </p>

      <div className="mt-10 space-y-12">
        {examples.map((ex) => (
          <section key={ex.slug}>
            {/* Header row — compact */}
            <div className="flex items-center gap-4 mb-3 flex-wrap">
              <h2 className="text-lg font-bold">{ex.title}</h2>
              <div className="h-4 w-px bg-fd-border" />
              <span className="text-xs text-fd-muted-foreground">{ex.description}</span>
              <div className="ml-auto flex items-center gap-3">
                <div className="flex items-center gap-1.5 rounded-md border border-fd-border bg-fd-card px-3 py-1.5">
                  <code className="font-mono text-[11px] text-fd-primary">{ex.command}</code>
                  <CopyButton text={ex.command} />
                </div>
                <a
                  href={ex.previewUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1 text-xs text-fd-muted-foreground hover:text-fd-foreground transition-colors"
                  title="Open in new tab"
                >
                  <ExternalLink className="size-3.5" />
                </a>
                <a
                  href={ex.source}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1 text-xs text-fd-muted-foreground hover:text-fd-foreground transition-colors"
                >
                  <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                  </svg>
                </a>
              </div>
            </div>

            <IframePreview iframeSrc={ex.previewUrl} codePath={ex.codePath} />
          </section>
        ))}
      </div>
    </div>
  );
}
